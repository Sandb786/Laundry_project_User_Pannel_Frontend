      ...
      <div className="min-h-fit bg-gray-100 border-1 border-white rounded-2xl flex flex-col items-center py-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
          {services.map((service, index) => (
            <motion.div
              key={index}
              className="md:w-38 w-42 p-4 bg-white border-4 border-white shadow-xl shadow-gray-300 rounded-2xl flex flex-col items-center cursor-pointer"
              onClick={() => handleClick(index)}
              initial={{ scale: 1 }}
              animate={{ scale: selected === index ? 0.85 : 1 }}
              transition={{ type: "spring", stiffness: 300, damping: 10 }}
            >
              <img src={service.image} alt={service.name} className="md:h-18 h-20 w-24 mb-3" />
